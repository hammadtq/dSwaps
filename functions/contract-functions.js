...
window.tokenToToken = async function(sourceAddress, destAddress, tokenAmount) {
  ...
  let txObject;
	...
	var tokenAmount = (
		new BN(tokenAmount).mul(web3.utils.toBN(power))
	).toString();
...
window.getEtherToTokenOrderList = async function(tokenContractAddress, tokenAddress){
	...
	try {
		...
		for (var i = 0; i < orderList.length; i++) {
			...
			var tokenAmount = (
				new BN(orderDetails._dstAmount).div(web3.utils.toBN(power))
		  ).toString();
...
window.getTokenToEtherOrderList = async function(tokenContractAddress, tokenAddress){
	...
	try {
		...
		for (var i = 0; i < orderList.length; i++) {
			...
			var tokenAmount = (
				new BN(orderDetails._srcAmount).div(web3.utils.toBN(power))
		  ).toString();
